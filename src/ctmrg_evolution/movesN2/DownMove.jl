...
include("isometryDown.jl")
# include("find_value.jl")

function DownMove!(C,T,tens,gt::Matrix{Symbol},chi::Int64,theta::Int64,N::Int64)
    ...
    pt1,p1 = isometryDown(C,T,tens,gt,i+dr[theta][1],j+dr[theta][2],chi,N);
    setproperty!(P, gt[f(i+dr[theta][1]),f(j+dr[theta][2])],p1);
    setproperty!(Ptilde, gt[f(i+dr[theta][1]),f(j+dr[theta][2])],pt1); 

    pt2,p2 = isometryDown(C,T,tens,gt,i,j,chi,N);
