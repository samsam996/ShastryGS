...
include("isometryRight.jl")

function RightMove!(C,T,tens,gt::Matrix{Symbol},chi::Int64,theta::Int64,N::Int64)
    
    P = ShastryGS.lattice(N)
    Ptilde = ShastryGS.lattice(N)
    ...
    i = N
    j = 1
    pt1,p1 = isometryRight(C,T,tens,gt,i+dr[theta][1],j+dr[theta][2],chi,N);
    setproperty!(P, gt[f(i+dr[theta][1]),f(j+dr[theta][2])],p1);
    setproperty!(Ptilde, gt[f(i+dr[theta][1]),f(j+dr[theta][2])],pt1); 

    pt2,p2 = isometryRight(C,T,tens,gt,i,j,chi,N);
    ...
    for i = N
        ...
        C_tmp = [ShastryGS.lattice(N) for k = 1:4]
        T_tmp = [ShastryGS.lattice(N) for k = 1:4]
       ...
       for j = 1:N
            ...
            value1,value2,value3 = find_value(C,T,P,Ptilde,gt,tens,del,dr,theta,mp,mm,i,j,N)
