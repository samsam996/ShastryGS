...
function OBC_PEPS(tens_A,cxd,cyd,gt::Matrix{Symbol},N::Int64)

    f(x) = mod(x-1,N) + 1

    C = [ShastryGS.lattice(N) for k = 1:4]
    T = [ShastryGS.lattice(N) for k = 1:4]
    ...
    for i = 1:N
        for j = 1:1
            ...
            # t1 = ITensor((ind2'), dag(ind1'), (ind3'), dag(ind2), (ind1), dag(ind3))
            # # @show inds(tt1)
            # # @show inds(t1)
            # for i = 1:dim(ind1)
            #     t1[ind1'=>i, ind2'=>i, ind3'=>i, dag(ind1)=>i, dag(ind2)=>i, dag(ind3)=>i] = 1
            # end
            t1 = t1*cy2
