...
function fastfullupdate(temperature,J1::Float64,J2::Float64,h::Float64,hs::Float64,dbetafu::Float64,C ,
    T,tens_a ,tens_A ,cxd, cyd ,physical_legs, ancilla_legs, gt::Matrix{Symbol},gg::Matrix{Symbol},
    chi::Int64,precision_ctm::Float64,beta::Float64,D::Int64, free_energy)
    ...
    bc = "no condition"
    C, T, iter,err_ctm = ctm!(tens_a,tens_A,cxd,cyd,gt,physical_legs,ancilla_legs,chi,precision_ctm,C,T,bc) 
    ...
    while temp > temperature[end] 
        ...
        if (temperature[relevant] <= tempprev && temperature[relevant] > temp) 
            ...
            PPS = 1/2*(PartitionPerSite(T,C,tens_a,gt,1,1) ) + 2*free_energy

            @show push!(PPSf, real(PPS))
        ...
        j = 1;
        for i = 1:N
            tens_a_left,tens_a_right, free_energy = full_update_x!(tens_a,tens_A,cxd,cyd,gt,physical_legs,ancilla_legs,gx,gy,D,C,T,i,j,free_energy)
            @showtime LeftMove!(C,T,tens_a_left,gt,chi,4)
            renormalisation!(C,T,gt)  
            @showtime RightMove!(C,T,tens_a_right,gt,chi,2)
            renormalisation!(C,T,gt)    
        end

        i = 1;
        for j = 1:N
            tens_a_up, tens_a_down, free_energy = full_update_y!(tens_a,tens_A,cxd,cyd,gt,physical_legs,ancilla_legs,gx,gy,D,C,T,i,j,free_energy)
            @showtime UpMove!(C,T,tens_a_up,gt,chi,1)
            renormalisation!(C,T,gt)  
            @showtime DownMove!(C,T,tens_a_down,gt,chi,3)
            renormalisation!(C,T,gt)    
        end
        ...
        tens_a, tens_A, free_energy = renormalisation_tens_A(tens_a,tens_A,cxd,cyd,physical_legs,ancilla_legs,gt,N, free_energy)

        tempprev = 1/beta
        beta = beta + dbetafu
        temp = 1/beta 

        name_data = string("Results/U1_ResultsFUD",string(D),"_TMPSAVE_","_nfu_",string(nfu),"hs",string(round(1e4*hs,digits = 3)),"h",string(round(1e2*h,digits = 3)),".jld2")
        save(name_data,
            "C",C,
            "T",T,
            "tens_A",tens_A,
