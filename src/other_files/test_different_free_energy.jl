...
for i in eachindex(tt)
    ...
    @show i
    file_name = "D13chi40/LocalTensorsD7temp"*tt[i]*"J0.63h100.0.jld2"
    ...
    filen_ame = "D13chi40/U1_ResultsD"*string(D)*"temp"*string(round(temp,digits = 5))*"_nsu_"*string(nsu),"hs"*string(round(1e4*hs,digits = 3))*"h"*string(round(1e2*h,digits = 3))*".jld2";
    D,magne,J2,h,J1,hs,nsu,energie,z2,temp,numb_it,chi,err_ctm,C,T,tens_A,cxd,cyd,PPS = 
    load(file_name,"D","magne","J2","h","J1","hs","nsu","energie","z2","tempe","numb_iter","chi","err_ctm",    
    "C","T","tens_A","cxd","cyd","PPS","gt")
